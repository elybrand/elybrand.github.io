---
layout: single
title: Something about Quantization
author_profile: true
---
...
We're still not done quantizing, since these point samples take on values
...
that the quantization error, or *distortion*, is \\( \delta = B 2^{-\frac{\mathcal{R}}{m}}\\). In other words, the distortion using MSQ decays
...
that for MSQ, distortion as a function of \\(m\\) cannot decay faster than \\(O(m^{-1})\\). There are, however, quantization schemes which, like MSQ, enjoy
...
\\(r\\) is any positive integer. Further, unlike MSQ, these encoding schemes "remember" quantization error.
...
proved for this holds for a particular family of \\(\rho\_r\\), namely
\\[ \rho_r(u\_{i-1},..., u\_{i-r}, y\_{i},..., y\_{i-r+1}) = y\_i + \sum\_{j=1}^{r} (-1)^{j-1} {r\choose j} u\_{i-j}. \\]
...
and for \\( r = 2 \\) we have
\\[ \begin{align}
q\_i = Q\_{\mathcal{A}}\left(\sum\_{j=1}^{r+1} j y\_{i-j+1} - \sum\_{j=1}^{r} j q\_{i-j} \right)
\end{align}\\]
...
and \\(q_{i-1}\\) is significant due to quantization error, the rate of change \\(\sum\_{j=1}^{r+1} y\_{i-j+1} - \sum\_{j=1}^{r} q\_{i-j}\\) isn't likely
to be as large as that of \\(y\_{i-1} - q\_{i-1} = (\Delta^r u)\_i\\). Appealing to mathematical intuition, this is simply because integration
...
is going to have low frequencies compared to the quantization error encoded in \\(u\\). Pushing error into higher frequencies is, after all,
...
error (in Euclidean norm) for recovering \\(y\in \mathbb{R}^m\\) from \\(q\\) decays like \\(O(m^{-r})\\). 
